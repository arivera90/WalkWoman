...
class Sound():
    ...
    def __init__(self):
        ...
        with resources.path(Config.get_instance().data["sound"]["background_music_pakage"],Config.get_instance().data["sound"]["background_music_filename"]) as background_music_path:
            self.__background_music =  pygame.mixer.music.load(background_music_path)
            self.__previous_position_x , self.__previous_position_y= Config.get_instance().data["sprite"]["woman_position"]
            self.__step_cadency = Config.get_instance().data["sound"]["walk_sound_cadency"] * Config.get_instance().data["sprite"]["delta_time"]
